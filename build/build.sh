...
# Get the git commit
if [ -f $GOPATH/src/github.com/openebs/openebs-exporter/GITCOMMIT ];
then
    GIT_COMMIT="$(cat $GOPATH/src/github.com/openebs/openebs-exporter/GITCOMMIT)"
else
...
env GOOS=$GOOS GOARCH=$GOARCH CGO_ENABLED=0  go build ${BUILD_TAG} -ldflags \
    "-X github.com/openebs/openebs-exporter/pkg/version.GitCommit=${GIT_COMMIT} \
    -X main.CtlName='${CTLNAME}' \
    -X github.com/openebs/openebs-exporter/pkg/version.Version=${VERSION}"\
    -o $output_name \
    ./cmd
