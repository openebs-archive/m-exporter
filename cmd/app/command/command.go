...
import (
	...
	collector "github.com/openebs/exporter/pkg/collector"
	"github.com/openebs/exporter/pkg/collector/pool"
	"github.com/openebs/exporter/pkg/collector/zvol"
	exec "github.com/openebs/exporter/pkg/exec"
...
func NewCmdVolumeExporter() (*cobra.Command, error) {
	...
	cmd := &cobra.Command{
		...
		Long: `openebs-exporter can be used to monitor openebs volumes and pools.
It can be deployed alongside the openebs volume or pool containers as sidecars.`,
		Example: `openebs-exporter -a=http://localhost:8001 -c=:9500 -m=/metrics`,
...
func Run(cmd *cobra.Command, options *VolumeExporterOptions) error {
	klog.Infof("Starting openebs-exporter ...")
	switch options.CASType {
	case "cstor":
		klog.Infof("Initialising openebs-exporter for the cstor")
		options.RegisterCstor()
	case "jiva":
		klog.Infof("Initialising openebs-exporter for the jiva")
		if err := options.RegisterJiva(); err != nil {
			klog.Fatal(err)
			return nil
		}
	case "pool":
		klog.Infof("Initialising openebs-exporter for the cstor pool")
		options.RegisterPool()
	default:
		return errors.New("unsupported CAS")
	}
	options.StartExporter()
	return nil
}
...
func (o *VolumeExporterOptions) RegisterJiva() error {
	...
	klog.Info("Registered openebs exporter for jiva")
	return nil
}
...
func (o *VolumeExporterOptions) RegisterCstor() {
	...
	prometheus.MustRegister(exporter)
	klog.Info("Registered openebs exporter for cstor")
	return
}
...
func (o *VolumeExporterOptions) RegisterPool() {
	...
	klog.Info("Registered openebs exporter for cstor pool")
	return
}

func buildRunner(timeout time.Duration, cmd string, args ...string) exec.Runner {
