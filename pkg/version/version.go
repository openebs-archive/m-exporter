...
const (
	versionFile   string = "/src/github.com/openebs/openebs-exporter/VERSION"
	buildMetaFile string = "/src/github.com/openebs/openebs-exporter/BUILDMETA"
...
// Current returns the current version of openebs-exporter
func Current() string {
	return GetVersion()
}
...
// Note: program name should be similar to openebs_exporter (with
...
func NewVersionCollector(program string) *prometheus.GaugeVec {
	buildInfo := prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			...
			Subsystem: program,
			Name:      "version",
			Help:      "A metric with a constant '1' value labeled by commit and version from which openebs-exporter was built.",
		},
