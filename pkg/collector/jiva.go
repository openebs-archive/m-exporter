...
import (
	...
	v1 "github.com/openebs/exporter/pkg/api"
	"github.com/openebs/exporter/pkg/util"
...
func (j *jiva) parse(volStats v1.VolumeStats, metrics *metrics) stats {
	...
	stats.logicalSize, _ = util.DivideFloat64(uBlocks, v1.BytesToGB)
	aUsed := parseFloat64(volStats.UsedLogicalBlocks, metrics)
	aUsed = aUsed * stats.sectorSize
	stats.actualSize, _ = util.DivideFloat64(aUsed, v1.BytesToGB)
	size := parseFloat64(volStats.Size, metrics)
	stats.size, _ = util.DivideFloat64(size, v1.BytesToGB)
