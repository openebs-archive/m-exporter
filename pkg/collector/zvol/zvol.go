...
import (
	...
	col "github.com/openebs/exporter/pkg/collector"
	types "github.com/openebs/exporter/pkg/exec"
...
func (v *volume) checkError(stdout []byte, ch chan<- prometheus.Metric) error {
	if IsNotInitialized(string(stdout)) {
		v.zfsStatsInitializeLibuzfsClientErrorCounter.Inc()
		v.zfsStatsInitializeLibuzfsClientErrorCounter.Collect(ch)
		return errors.New(InitializeLibuzfsClientErr.String())
	}

	if IsNoDataSetAvailable(string(stdout)) {
		v.zfsStatsNoDataSetAvailableErrorCounter.Inc()
		v.zfsStatsNoDataSetAvailableErrorCounter.Collect(ch)
		return errors.New(NoDataSetAvailable.String())
	}
	return nil
}

func (v *volume) get(ch chan<- prometheus.Metric) (Stats, error) {
	var (
		err    error
		stdout []byte
		stats  = Stats{}
...
func (v *volume) setZVolStats(stats Stats) {
