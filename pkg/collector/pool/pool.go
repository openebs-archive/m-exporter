...
func (p *pool) checkError(stdout []byte, ch chan<- prometheus.Metric) error {
	...
	if IsNotAvailable(string(stdout)) {
		p.noPoolAvailableErrorCounter.Inc()
		p.noPoolAvailableErrorCounter.Collect(ch)
		return errors.New(NoPoolAvailable.String())
	}
	return nil
}

func (p *pool) getZpoolStats(ch chan<- prometheus.Metric) (Stats, error) {
	var (
		err         error
		stdoutZpool []byte
		zpoolStats  = Stats{}
	...
	zpoolStats, err = ListParser(stdoutZpool)
	if err != nil {
